...
export default function BookLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <header className="glass-frosted sticky top-0 z-50 border-b border-border">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2 sm:gap-3">
            <Image src="/assets/wc-logo-white.png" alt="Westminster Chariots" width={32} height={32} className="h-7 sm:h-8 w-auto" />
              ...
              <div key={step} className="flex items-center">
                {i < currentStep ? (
                  <Link href={STEP_ROUTES[i]} className="flex items-center gap-2 group cursor-pointer">
                    ...
                    <span className="text-xs font-body text-muted-foreground group-hover:text-foreground transition-colors">{step}</span>
                  </Link>
                ) : (
                  <div className="flex items-center gap-2">
                    <div className={`h-6 w-6 rounded-full flex items-center justify-center text-[10px] font-body font-semibold border transition-all ${
                      i === currentStep
          ...
          <div className="flex md:hidden items-center gap-1.5">
            {STEP_LABELS.map((step, i) => (
              <div key={step} className={`rounded-full transition-all ${
                i === currentStep ? 'h-2 w-6 bg-primary' : i < currentStep ? 'h-2 w-2 bg-primary/10' : 'h-2 w-2 bg-border'
              }`} />
            ))}
          </div>

          <a href="tel:+15714351832" className="text-xs text-muted-foreground font-body hover:text-primary transition-colors flex items-center gap-1.5">
