    ...
    try {
      ...
      if (booking.driverId) {
        ...
        const driverResponse = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL || "https://wc-backend-ayx0.onrender.com/api/v1"}/drivers/${booking.driverId}`,
          {
  ...
  return (
    <section className="space-y-4 md:space-y-6">
      ...
      <div className="rounded-2xl md:rounded-4xl bg-white shadow-sm ring-1 ring-slate-200 p-4 md:p-5">
        ...
        <div className="mt-4 md:mt-5 space-y-3 md:space-y-0 md:grid md:gap-4 lg:grid-cols-[1fr_1.8fr] items-center">
          ...
          <div className="flex flex-wrap gap-2">
            {[
              ...
              <button
                ...
                className={`px-3 py-1.5 md:px-4 md:py-2 rounded-full text-xs font-semibold transition ${
      ...
      {loading ? (
        <div className="space-y-3 md:space-y-4">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-20 md:h-28 rounded-2xl md:rounded-3xl bg-slate-100 animate-pulse" />
          ))}
        </div>
        ...
        <div className="space-y-3 md:space-y-4">
          {paginatedBookings.map((booking) => {
            ...
            return (
              <div key={booking.id} className="rounded-2xl md:rounded-4xl bg-slate-50 p-4 md:p-6 shadow-sm ring-1 ring-slate-200 transition hover:-translate-y-0.5 hover:shadow-md">
                {/* Mobile Layout */}
                <div className="md:hidden space-y-3">
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className={`rounded-full px-2 py-1 text-[10px] font-semibold ${statusColors[booking.status] || statusColors.pending}`}>
                          {statusTitles[booking.status] || booking.status.replace(/_/g, " ")}
                        </span>
                      </div>
                      <p className="mt-1 text-xs text-slate-600">{statusNextSteps[booking.status]}</p>
                    </div>
                    <Button variant="outline" size="sm" onClick={() => handleBookingSelect(booking)} className="gap-1 text-xs">
                      Details
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="rounded-xl bg-white p-3 ring-1 ring-slate-200">
                      <p className="text-[10px] uppercase tracking-[0.3em] text-slate-500">Pickup</p>
                      <p className="mt-1 text-xs text-slate-900 truncate">{booking.pickupLocation}</p>
                    </div>
                    <div className="rounded-xl bg-white p-3 ring-1 ring-slate-200">
                      <p className="text-[10px] uppercase tracking-[0.3em] text-slate-500">Drop-off</p>
                      <p className="mt-1 text-xs text-slate-900 truncate">{booking.dropoffLocation}</p>
                    </div>
                  </div>

                  <div className="grid grid-cols-2 gap-2">
                    <div className="rounded-xl bg-white p-3 ring-1 ring-slate-200">
                      <p className="text-[10px] uppercase tracking-[0.3em] text-slate-500">Time</p>
                    ...
                    <div className="rounded-xl bg-white p-3 ring-1 ring-slate-200">
                      <p className="text-[10px] uppercase tracking-[0.3em] text-slate-500">Vehicle</p>
                      <div className="mt-2 flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-xs font-semibold text-slate-900">{vehicleNames[booking.vehicleType]}</p>
                          <p className="text-[10px] uppercase tracking-[0.24em] text-slate-500">{booking.vehicleType}</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-slate-200">
                    <div>
                      <p className="text-lg font-display font-semibold text-slate-900">${booking.totalPrice ? Number(booking.totalPrice).toFixed(0) : "0"}</p>
                      <p className="text-[10px] uppercase tracking-[0.24em] text-slate-500">{booking.vehicleType}</p>
                    </div>
                    <div className="flex gap-2">
                      ...
                      <AlertDialog>
                        <AlertDialogTrigger asChild>
                          <Button variant="ghost" size="sm" className="gap-1 text-xs text-destructive hover:bg-destructive/10">
                ...
                <>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                    <div className="rounded-3xl bg-white p-4 ring-1 ring-slate-200">
                      ...
                      {vehicleInfo ? (
                        <div className="mt-2">
                          <p className="text-sm font-semibold text-slate-900">{vehicleInfo.make} {vehicleInfo.model}{vehicleInfo.year ? ` (${vehicleInfo.year})` : ''}</p>
                          {vehicleInfo.imageUrl && (
                            <div className="mt-3 relative h-32 w-full rounded-xl overflow-hidden bg-slate-100">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="rounded-3xl bg-white p-4 ring-1 ring-slate-200">
                      ...
                      {driverInfo ? (
                        ...
                        <p className="mt-2 text-sm text-slate-700">No driver assigned yet</p>
                      )}
                    </div>
                    <div className="rounded-3xl bg-white p-4 ring-1 ring-slate-200">
                  ...
                  <div className="rounded-3xl bg-white p-4 ring-1 ring-slate-200">
                    <p className="text-[11px] uppercase tracking-[0.24em] text-slate-500">Notes & Requests</p>
                    <div className="mt-2 space-y-3">
                      <p className="text-sm text-slate-700">{selectedBooking.specialRequests || "No special requests."}</p>
                      {selectedBooking.flightNumber && (
                        <div className="pt-2 border-t border-slate-200">
                          <p className="text-sm text-slate-700"><span className="font-semibold">Flight Number:</span> {selectedBooking.flightNumber}</p>
                        </div>
                      )}
                      {selectedBooking.dispatcherNotes && (
                        <div className="pt-2 border-t border-slate-200">
                          <p className="text-sm text-slate-700"><span className="font-semibold">Dispatcher Notes:</span> {selectedBooking.dispatcherNotes}</p>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="rounded-3xl bg-white p-4 ring-1 ring-slate-200">
                    <p className="text-[11px] uppercase tracking-[0.24em] text-slate-500">Booking Information</p>
                    <div className="mt-2 grid grid-cols-2 gap-3">
                      <div>
                        <p className="text-xs text-slate-500">Created</p>
                        <p className="text-sm text-slate-900">{selectedBooking.createdAt ? format(parseISO(selectedBooking.createdAt), "MMM d, yyyy HH:mm") : "—"}</p>
                      </div>
                      <div>
                        <p className="text-xs text-slate-500">Last Updated</p>
                        <p className="text-sm text-slate-900">{selectedBooking.updatedAt ? format(parseISO(selectedBooking.updatedAt), "MMM d, yyyy HH:mm") : "—"}</p>
                      </div>
                    </div>
                  </div>
                </>
      ...
      {!loading && searchedBookings.length > 0 && (
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center justify-between mt-4 pt-4 border-t border-slate-200 text-slate-600">
          ...
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" size="sm" disabled={page === 0} onClick={() => onPageChange(Math.max(page - 1, 0))} className="text-xs">
              Previous
            </Button>
            <Button variant="outline" size="sm" disabled={page + 1 >= totalPages} onClick={() => onPageChange(page + 1)} className="text-xs">
              Next
            </Button>
          </div>
        </div>
