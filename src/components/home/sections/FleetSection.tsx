    ...
    <AnimatePresence>
      ...
      >
        ...
        <motion.div
          ...
          className="relative z-10 w-full max-w-3xl overflow-hidden rounded-3xl border border-white/10 bg-[#111118] shadow-[0_40px_100px_rgba(0,0,0,0.7)]"
        ...
        >
          ...
          {/* Image — light background so dark vehicles are visible */}
          <div
            className="relative h-56 overflow-hidden sm:h-72"
            style={{ background: "radial-gradient(ellipse at 50% 60%, #1c2540 0%, #0e1120 55%, #111118 100%)" }}
          >
            {/* Subtle glow under the vehicle */}
            <div className="pointer-events-none absolute bottom-0 left-1/2 h-32 w-3/4 -translate-x-1/2 rounded-full bg-blue-600/10 blur-3xl" />
            <Image
              src={vehicle.image}
              alt={vehicle.name}
              fill
              className="object-contain object-center px-6 pb-4 pt-6"
              sizes="(max-width: 768px) 100vw, 768px"
            />
            <div className="absolute inset-x-0 bottom-0 h-20 bg-gradient-to-t from-[#111118] to-transparent" />
            <div className="absolute bottom-4 left-6">
              <span className="rounded-full border border-white/20 bg-black/50 px-3 py-1 text-[10px] font-medium uppercase tracking-[0.3em] text-white/80 backdrop-blur-md">
                {vehicle.name}
              </span>
            </div>
          </div>
          ...
          <div className="p-6 sm:p-8">
            ...
            <div className="mt-6 flex flex-wrap items-center gap-4 border-t border-white/8 pt-6">
              <div className="flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm text-foreground/80">
            ...
            {extras && (
              <div className="mt-5">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {extras.models.map((m) => (
                    <span key={m} className="rounded-md border border-white/8 bg-white/5 px-3 py-1 text-sm text-foreground/70">
                      {m}
                    </span>
                  ))}
                </div>
...
function FleetCard({ v, index, onClick }: { v: Vehicle; index: number; onClick: () => void }) {
  return (
    ...
    >
      <button
        onClick={onClick}
        className="group relative block w-full overflow-hidden rounded-3xl border border-white/8 bg-card/30 backdrop-blur-md transition-all duration-500 hover:border-white/20 hover:bg-card/50 text-left cursor-pointer"
      >
